// config.js — shared Supabase client + helpers for both admin.html and user.html.
// Imported as an ES module: <script type="module"> import { db, ... } from './config.js'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

// ─── FILL THESE IN ───────────────────────────────────────────
// Project settings → API. The anon key is safe to ship client-side;
// Row Level Security in schema.sql is what actually protects the data.
const SUPABASE_URL = 'https://vtiobzmsalsvwocvlotm.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_BfCizLoxWkUk2L9sUXzv9w_M5TdA3uD';
// ─────────────────────────────────────────────────────────────

export const db = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

// A throwaway auth client used by admins when creating accounts.
// It prevents the admin's own browser session from being replaced by the new user session.
export function newAuthClient() {
  return createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: {
      persistSession: false,
      autoRefreshToken: false,
      detectSessionInUrl: false,
    },
  });
}

function cleanSignupProfile({ email, full_name, username, mobile_number } = {}) {
  return {
    email: String(email || '').trim(),
    full_name: String(full_name || '').trim(),
    username: String(username || '').trim(),
    mobile_number: String(mobile_number || '').trim(),
  };
}

export async function signUpWithProfile(authClient, { email, password, full_name, username, mobile_number = '', emailRedirectTo } = {}) {
  const clean = cleanSignupProfile({ email, full_name, username, mobile_number });
  const metadata = {
    full_name: clean.full_name,
    username: clean.username,
  };
  if (clean.mobile_number) metadata.mobile_number = clean.mobile_number;

  const result = await authClient.auth.signUp({
    email: clean.email,
    password,
    options: { data: metadata, emailRedirectTo },
  });
  if (result.error) return result;

  const userId = result.data?.user?.id;
  if (userId) {
    await db.rpc('sync_signup_profile', {
      p_user_id: userId,
      p_email: clean.email,
      p_full_name: clean.full_name,
      p_username: clean.username,
      p_mobile_number: clean.mobile_number || null,
    }).then(() => {}, () => {});
  }
  return result;
}

// Sign in with an email OR a username (shared by index/user/admin login forms).
// Usernames resolve server-side through the login-with-username edge function,
// so the browser never calls the enumerable get_email_for_username RPC. While
// the function isn't deployed yet (or is unreachable) we fall back to the old
// RPC path, so login keeps working during the transition.
export async function signInWithIdentifier(rawIdent, password) {
  const ident = String(rawIdent || '').trim();
  if (ident.includes('@')) return db.auth.signInWithPassword({ email: ident, password });

  const { data, error } = await db.functions.invoke('login-with-username', {
    body: { username: ident, password },
  });
  if (!error) {
    const payload = typeof data === 'string' ? JSON.parse(data) : data;
    if (payload?.access_token && payload?.refresh_token) {
      return db.auth.setSession({
        access_token: payload.access_token,
        refresh_token: payload.refresh_token,
      });
    }
    return { data: null, error: { message: t('invalidCredentials') } };
  }
  // The function answered with 400/401/429 → real login failure, keep it generic.
  const status = error?.context?.status ?? error?.status;
  if (status === 400 || status === 401) return { data: null, error: { message: t('invalidCredentials') } };
  if (status === 429) return { data: null, error: { message: error.message || t('invalidCredentials') } };

  // Function missing / not deployed / network hiccup → legacy RPC fallback.
  const { data: resolved, error: rpcErr } = await db.rpc('get_email_for_username', { p_username: ident });
  if (rpcErr) return { data: null, error: rpcErr };
  if (!resolved) return { data: null, error: { message: t('userEmailNotFound') } };
  return db.auth.signInWithPassword({ email: resolved, password });
}

// Mask an email for display: "claude@gmail.zz" → "cl***@g***.zz".
// Superusers/sysadmins un-mask via the audited reveal_email() RPC.
export function maskEmail(email) {
  if (!email) return '';
  const [local = '', domain = ''] = String(email).split('@');
  if (!domain) return local.slice(0, 2) + '***';
  const dot = domain.lastIndexOf('.');
  const tld = dot > 0 ? domain.slice(dot) : '';
  return `${local.slice(0, 2)}***@${domain.slice(0, 1)}***${tld}`;
}

// ── anonymous varsling case codes ──────────────────────────────
// The code is the reporter's ONLY key to their case. It is generated and
// hashed in the browser; only the sha256 hex (anon_id) is ever transmitted.
const CASE_ALPHABET = 'ABCDEFGHJKMNPQRSTVWXYZ23456789'; // no I/L/O/U/0/1 — safe to write down

export function generateCaseCode() {
  const bytes = new Uint8Array(20);
  crypto.getRandomValues(bytes);
  let out = '';
  for (const b of bytes) out += CASE_ALPHABET[b % CASE_ALPHABET.length]; // ~98 bits; modulo bias irrelevant at this size
  return 'HMS-' + out.match(/.{5}/g).join('-');
}

export async function caseCodeHash(code) {
  const clean = String(code || '').toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/^HMS/, '');
  if (!clean) return null;
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(clean));
  return [...new Uint8Array(buf)].map(b => b.toString(16).padStart(2, '0')).join('');
}

// Call an edge function WITHOUT the user's JWT (anon key only) — required for
// the anonymous varsling path so API logs never tie a user to the request.
// Never replace this with db.functions.invoke (it attaches the session JWT).
// ── web push ───────────────────────────────────────────────────
// Public half of the VAPID key pair. Generate once with
// `npx web-push generate-vapid-keys`, paste the PUBLIC key here and set
// VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY / VAPID_SUBJECT as secrets on the
// send-push edge function. Empty string = push UI shows "not configured".
export const VAPID_PUBLIC_KEY = 'BKWQo8AiWJxqVpkVKsQqsE1vkmLJn46bWBpIe6y8aqBz9dpVARS6zCwnfJhVQD_kD9bVuyK7zv9-iKIUi2hQRrM';

export function urlBase64ToUint8Array(base64) {
  const padding = '='.repeat((4 - base64.length % 4) % 4);
  const b64 = (base64 + padding).replace(/-/g, '+').replace(/_/g, '/');
  const raw = atob(b64);
  return Uint8Array.from([...raw].map(c => c.charCodeAt(0)));
}

export async function callPublicFunction(name, body) {
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/${name}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        apikey: SUPABASE_ANON_KEY,
        Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      },
      body: JSON.stringify(body || {}),
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return { data: null, error: { message: data.error || `Request failed (${res.status})`, status: res.status } };
    return { data, error: null };
  } catch (e) {
    return { data: null, error: { message: e.message || 'Network error' } };
  }
}

// Return the signed-in user's profile (with role), or null.
export async function currentProfile() {
  const { data: { user } } = await db.auth.getUser();
  if (!user) return null;
  const { data } = await db.from('profiles').select('*').eq('id', user.id).single();
  return data;
}

// ── Role hierarchy ─────────────────────────────────────────────
//   sysadmin > superuser > admin > verneombud > user
//   'super_admin' is the legacy value (pre-migration) and ranks as sysadmin
//   so the new client never locks the top tier out before the SQL migration runs.
export const ROLE_RANK = { user: 0, verneombud: 1, admin: 2, superuser: 3, sysadmin: 4, super_admin: 4 };
export const roleRank = (p) => ROLE_RANK[typeof p === 'string' ? p : p?.role] ?? 0;
export const isSysadmin   = (p) => roleRank(p) >= 4;            // programmers; hidden top tier
export const isSuperPlus  = (p) => roleRank(p) >= 3;            // superuser & sysadmin — full power
export const isAdminPlus  = (p) => roleRank(p) >= 2;            // admin and above
export const canViewChecklistLogs = (p) => roleRank(p) >= 1;   // verneombud and above
export const canAccessVarsling = (p) => isAdminPlus(p) && !!(typeof p === 'object' && p?.can_access_varsling);
export const isSuspended = (p) => !!(typeof p === 'object' && p?.is_suspended);

// Guard a page: ensure someone is signed in and meets a minimum rank.
//   admin.html passes { admin: true } → verneombud (rank 1) or above may enter;
//   the admin UI then restricts each tier to what it may see.
export async function requireSession({ admin = false, minRank = 0 } = {}) {
  const profile = await currentProfile();
  if (!profile) return { ok: false, reason: 'no-session' };
  const required = admin ? 1 : minRank;
  if (roleRank(profile) < required) return { ok: false, reason: 'not-admin', profile };
  return { ok: true, profile };
}

// ── Cross-page prefetch cache ──────────────────────────────────
// index.html warms this right after the profile is known, so user.html can
// paint its groups without waiting on the network (index and user.html are
// separate documents, so the data rides across the navigation in sessionStorage,
// keyed to the user id). Stale-while-revalidate: user.html still refetches in
// the background and repaints only if something actually changed.
const PREFETCH_KEY = 'hms:pf:worker';
const PREFETCH_TTL = 5 * 60 * 1000; // ignore anything older than 5 min

function mergePrefetch(uid, patch) {
  let cur = {};
  try { const raw = sessionStorage.getItem(PREFETCH_KEY); if (raw) cur = JSON.parse(raw); } catch (e) { /* ignore */ }
  if (cur.uid !== uid) cur = { uid };
  const next = { ...cur, ...patch, uid, t: Date.now() };
  try { sessionStorage.setItem(PREFETCH_KEY, JSON.stringify(next)); } catch (e) { /* storage may be unavailable */ }
}

// Returns { groups?, departmentIds?, uid, t } for this user if fresh, else null.
export function readWorkerPrefetch(uid, { maxAgeMs = PREFETCH_TTL } = {}) {
  try {
    const raw = sessionStorage.getItem(PREFETCH_KEY);
    if (!raw) return null;
    const p = JSON.parse(raw);
    if (!uid || p.uid !== uid) return null;
    if (Date.now() - (p.t || 0) > maxAgeMs) return null;
    return p;
  } catch (e) { return null; }
}

export function cacheWorkerGroups(uid, groups) { if (uid) mergePrefetch(uid, { groups: groups || [] }); }
export function cacheWorkerDepartments(uid, ids) { if (uid) mergePrefetch(uid, { departmentIds: ids || [] }); }

// Warm the worker cache (main groups + department memberships) in parallel.
// Never blocks the caller longer than timeoutMs — a slow network must not stall
// the redirect; user.html falls back to a normal fetch on a cold cache.
export async function prefetchWorkerData(profile, { timeoutMs = 2500 } = {}) {
  if (!profile?.id) return;
  const uid = profile.id;
  const work = Promise.allSettled([
    db.from('groups').select('*').order('sort_order'),
    db.from('user_departments').select('department_id').eq('user_id', uid),
  ]).then(([g, d]) => {
    const patch = {};
    if (g.status === 'fulfilled' && !g.value.error) patch.groups = g.value.data || [];
    if (d.status === 'fulfilled' && !d.value.error) patch.departmentIds = (d.value.data || []).map(r => r.department_id);
    if (Object.keys(patch).length) mergePrefetch(uid, patch);
  }).catch(() => { /* prefetch is best-effort */ });
  await Promise.race([work, new Promise(r => setTimeout(r, timeoutMs))]);
}

// Tiny DOM helper: el('div', { class: 'x' }, [children]) or el('div', 'text').
export function el(tag, props = {}, kids = []) {
  const node = document.createElement(tag);
  if (typeof props === 'string') { node.textContent = props; return node; }
  for (const [k, v] of Object.entries(props)) {
    if (k === 'class') node.className = v;
    else if (k === 'html') node.innerHTML = v;
    else if (k.startsWith('on') && typeof v === 'function') node.addEventListener(k.slice(2), v);
    else if (v != null) node.setAttribute(k, v);
  }
  for (const kid of [].concat(kids)) {
    if (kid == null) continue;
    // Coerce anything that isn't a DOM node (numbers, booleans…) to text —
    // appendChild(number) throws and blanks the whole view otherwise.
    node.appendChild(kid instanceof Node ? kid : document.createTextNode(String(kid)));
  }
  return node;
}

export const $ = (sel, root = document) => root.querySelector(sel);

// <option> shorthand and a labelled form field — shared by the group modules.
export const opt = (v, label) => el('option', { value: v }, label);
export function labeled(text, inputEl, required = false) {
  return el('label', { class: 'field' },
    [el('span', {}, [text, required ? el('span', { class: 'req' }, ' *') : null]), inputEl]);
}

// Themed replacement for a native <input type=file> (the raw browser "Choose
// file" button doesn't follow the app's light/dark theme). Hides the real
// input and drives it from a styled button + filename readout kept in sync
// via 'change', so any existing onchange handler on `input` still fires.
export function fileButton(input, label) {
  const isImage = (input.getAttribute('accept') || '').includes('image');
  const nameEl = el('span', { class: 'file-btn-name' }, t('noFileChosen'));
  const sync = () => {
    const files = input.files;
    nameEl.textContent = !files || !files.length ? t('noFileChosen')
      : files.length === 1 ? files[0].name : t('filesChosenCount', { count: files.length });
  };
  input.style.display = 'none';
  input.addEventListener('change', sync);
  sync();
  const btn = el('button', { type: 'button', class: 'btn btn-ghost file-btn-trigger', onclick: () => input.click() }, [
    el('span', { class: 'file-btn-icon' }, isImage ? '📷' : '📎'),
    el('span', {}, label || t('chooseFile')),
  ]);
  return el('div', { class: 'file-btn' }, [btn, nameEl, input]);
}

let connectionStatus = typeof navigator === 'undefined' || navigator.onLine ? 'online' : 'offline';
let connectionMonitorStarted = false;
let connectionReconnectTimer = null;

function connectionStatusTitle(status) {
  switch (status) {
    case 'online': return 'Live and connected';
    case 'reconnecting': return 'Reconnecting...';
    case 'offline': return 'Offline';
    default: return '';
  }
}

function setConnectionStatus(status) {
  if (connectionStatus === status) return;
  connectionStatus = status;
  document.querySelectorAll('.status-dot').forEach(dot => {
    dot.className = `status-dot ${status}`;
    dot.title = connectionStatusTitle(status);
    dot.setAttribute('aria-label', connectionStatusTitle(status));
  });
}

async function checkConnection() {
  if (!navigator.onLine) {
    setConnectionStatus('offline');
    return;
  }
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 4000);
    const res = await fetch(`${SUPABASE_URL}/rest/v1/profiles?select=id&limit=1`, {
      method: 'GET',
      cache: 'no-store',
      headers: { apikey: SUPABASE_ANON_KEY, Accept: 'application/json' },
      signal: controller.signal,
    });
    clearTimeout(timeout);
    if (res.status < 500) {
      if (connectionReconnectTimer) clearTimeout(connectionReconnectTimer);
      connectionReconnectTimer = null;
      setConnectionStatus('online');
      return;
    }
    throw new Error(`Health check failed: ${res.status}`);
  } catch (err) {
    if (connectionStatus === 'online') {
      setConnectionStatus('reconnecting');
      if (!connectionReconnectTimer) {
        connectionReconnectTimer = setTimeout(() => {
          if (connectionStatus === 'reconnecting') setConnectionStatus('offline');
          connectionReconnectTimer = null;
        }, 12000);
      }
    } else if (connectionStatus !== 'reconnecting') {
      setConnectionStatus('offline');
    }
  }
}

export function startConnectionMonitor() {
  if (connectionMonitorStarted || typeof window === 'undefined') return;
  connectionMonitorStarted = true;
  window.addEventListener('online', () => {
    setConnectionStatus('reconnecting');
    checkConnection();
  });
  window.addEventListener('offline', () => {
    if (connectionReconnectTimer) clearTimeout(connectionReconnectTimer);
    connectionReconnectTimer = null;
    setConnectionStatus('offline');
  });
  checkConnection();
  setInterval(checkConnection, 15000);
}

export function connectionStatusDot() {
  startConnectionMonitor();
  return el('span', {
    class: `status-dot ${connectionStatus}`,
    title: connectionStatusTitle(connectionStatus),
    'aria-label': connectionStatusTitle(connectionStatus),
  });
}

export function initTheme() {
  const saved = localStorage.getItem('theme');
  const prefersDark = window.matchMedia?.('(prefers-color-scheme: dark)')?.matches;
  document.documentElement.dataset.theme = saved || (prefersDark ? 'dark' : 'light');
}

export function themeToggleButton() {
  initTheme();
  const btn = el('button', { class: 'iconbtn theme-toggle', title: t('toggleDarkMode'), onclick: toggleTheme }, themeIcon());
  function toggleTheme() {
    const next = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark';
    document.documentElement.dataset.theme = next;
    localStorage.setItem('theme', next);
    btn.textContent = themeIcon();
  }
  return btn;
}

function themeIcon() {
  return document.documentElement.dataset.theme === 'dark' ? '☀' : '◐';
}

const LANG_KEY = 'language';
const DEFAULT_LANG = 'en';

const TEXT = {
  en: {
    accounts: 'Accounts',
    addTitleAndField: 'Add a title and at least one field.',
    addTitleAndItem: 'Add a title and at least one item.',
    assignmentRemoved: 'Assignment removed',
    belowQuotaAcross: '{missed} missed across {days} days',
    caseResolved: 'Case resolved',
    changeRequestSent: 'Change request sent',
    checklistTypes: 'Checklist types',
    chooseCourseFirst: 'Choose a course first.',
    chooseFileFirst: 'Choose a file first.',
    chooseVisibleAssignee: 'Choose a visible assignee.',
    courseAssigned: 'Course assigned',
    currentWorkspace: 'Current workspace',
    deleted: 'Deleted',
    emailUsernameTempPasswordRequired: 'Email, username and temporary password are required.',
    nameEmailUsernamePasswordRequired: 'Name, email, username and password are required.',
    nameEmailUsernameTempPasswordRequired: 'Name, email, username and temporary password are required.',
    exampleKitchen: 'e.g. Kitchen',
    excelLibraryLoadFailed: 'Could not load the spreadsheet library.',
    expectedGot: 'expected {expected}, got {got}',
    expectedIssues: 'Expected issues',
    finalReportSaved: 'Final report saved',
    finalReportVisibleHint: 'The final report is visible to the reporter once saved.',
    folders: 'Folders',
    groupAdded: 'Group added',
    importBelowRoleOnly: 'You can only import users below your own role.',
    manageCourses: 'Manage courses',
    managementPanel: 'Management panel',
    medianResolve: 'Median resolve',
    migrationChecklistLogsMissing: 'Run the checklist logs migration to enable this.',
    migrationCoursesMissing: 'Run the courses migration to enable this.',
    missed: 'Missed',
    nothingToExport: 'Nothing to export.',
    notificationPreferenceSaved: 'Notification preference saved',
    oldestOpen: 'Oldest open',
    passwordMinLength: 'Password must be at least 6 characters.',
    profileChangeApproved: 'Profile change approved',
    profileChangeRejected: 'Profile change rejected',
    refresh: 'Refresh',
    reported: 'Reported',
    resolutionRate: 'Resolution rate',
    serious: 'Serious',
    shortCount: '{count} short',
    spreadsheetEmpty: 'The spreadsheet is empty.',
    uploaded: 'Uploaded',
    userAddedTempPassword: 'User added with a temporary password.',
    userCreatedNoId: 'User created, but no ID was returned.',
    workspace: 'Workspace',
    access: 'Access',
    accountsAccess: 'Accounts & access',
    accountCreatedGrantAccess: 'Account created. Confirm your email, then ask an admin to grant access.',
    accountCreatedWaitApproval: 'Account created. Confirm your email, then wait for admin approval.',
    actionLog: 'Action log',
    actionNotePlaceholder: 'Record an action taken or a note...',
    add: 'Add',
    addActionNote: 'Add action / note',
    addField: '+ Add field',
    addProcedureStep: '+ Add procedure step',
    addGroup: 'Add group',
    addItem: '+ Add item',
    addToLog: 'Add to log',
    addUser: '+ Add user',
    adminConsole: 'Admin console',
    adminConsoleMeta: 'Manage groups, incidents, and access',
    adminNote: 'Admin note',
    all: 'All',
    allChecklists: 'All checklists',
    apply: 'Apply',
    allTypes: 'All types',
    anonymous: 'Anonymous',
    admins: 'Admins',
    answerAllQuestions: 'Answer all questions ({count} left).',
    approved: 'Approved',
    archive: 'Archive',
    archived: 'Archived',
    archivedReports: 'Archived reports',
    assignmentCount: '{count} assigned',
    assignmentSaved: 'Assignment saved.',
    assignments: 'Assignments',
    assignmentsRemoved: '{count} assignment(s) removed.',
    assignCourse: 'Assign course',
    assignSelected: 'Assign selected',
    attemptCountPassedAverage: '{attempts} attempt(s) - {passed} passed - average {average}%',
    attempts: 'Attempts',
    averageScore: 'Average score',
    checklistLogs: 'Checklist logs',
    dateFrom: 'From',
    dateUntil: 'Until',
    dashboardPeriod: 'Dashboard period',
    periodPrompt: 'Use YYYY-MM-DD or YYYY-MM-DD..YYYY-MM-DD.',
    invalidPeriod: 'Use YYYY-MM-DD or YYYY-MM-DD..YYYY-MM-DD.',
    exportExcel: 'Export to Excel',
    importExcel: 'Import Excel',
    expected: 'Expected',
    expectedChecked: 'Expected checked state',
    expectedMax: 'Expected max',
    expectedMin: 'Expected min',
    expectedValues: 'Expected values',
    filterByType: 'Report type',
    filterByUser: 'User',
    hazard: 'Hazard',
    hazardConsequences: 'What could it lead to?',
    notifications: 'Notifications',
    notifyAll: 'Every report',
    notifyOff: 'Off',
    notifyRed: 'Emergencies only',
    reportHazard: 'Report hazard',
    roundNearest: 'Nearest',
    roundUp: 'Round up',
    sliderRound: 'Display',
    unarchive: 'Unarchive',
    whatHazard: 'What is the hazard?',
    whenSpotted: 'When did you notice it?',
    anyAssignee: 'Any assignee',
    anyCourse: 'Any course',
    anyForm: 'Any form',
    filterByForm: 'Form',
    form: 'Form',
    procedureLogsMeta: 'Submitted procedure forms',
    noProcedureSubmissions: 'No procedure submissions yet',
    noProcedureSubmissionsMatchFilters: 'No procedure submissions match these filters',
    anyReporter: 'Anyone',
    filterByAssignee: 'Assigned admin',
    filterByReporter: 'Reported by',
    inProgressAssignedToYou: 'In progress assigned to you',
    unassigned: 'Unassigned',
    assignedTo: 'Assigned to',
    attachments: 'Pictures',
    changeRequests: 'Change requests',
    checklist: 'Checklist',
    course: 'Course',
    procedure: 'Procedure',
    checklistComplete: 'Checklist submitted.',
    checklistTypeCheck: 'Check',
    checklistTypeChoice: 'Multiple choice',
    checklistTypeText: 'Text',
    checklistsCompletedInPeriod: 'Completed checklists',
    checklistsCompletedToday: 'Completed checklists today',
    checklistsToDo: 'Checklists to do',
    todaysChecklists: "Today's checklists",
    greetingMorning: 'Good morning',
    greetingAfternoon: 'Good afternoon',
    greetingEvening: 'Good evening',
    doneOfTotalToday: '{done} of {total} done today',
    allChecklistsDone: 'All checklists done for today',
    nothingScheduledToday: 'No checklists scheduled for today',
    quickActions: 'Quick actions',
    yourActivity: 'Your activity',
    myReports: 'My reports',
    communications: 'Communications',
    news: 'News',
    safetyNews: 'Safety news',
    safetyNewsToggle: 'Mark as safety news',
    noSafetyNews: 'No safety news.',
    newNews: 'New news',
    commTitle: 'Title',
    commBody: 'Message',
    commImage: 'Cover image (optional)',
    commAttachments: 'Attachments (optional)',
    broadcastAudience: 'Audience',
    audienceEveryone: 'Everyone',
    audienceDepartments: 'Specific departments',
    selectDepartments: 'Select departments',
    pickAtLeastOneDepartment: 'Select at least one department.',
    messageBodyRequired: 'Add a title or message first.',
    send: 'Send',
    sending: 'Sending…',
    communicationSent: 'Sent.',
    readReceipts: 'Read receipts',
    readByCount: '{read} of {total} read',
    readBy: 'Read by',
    notYetRead: 'Not yet read',
    unread: 'Unread',
    noCommunications: 'No communications yet.',
    noCommunicationsHint: 'News from your workplace appears here.',
    noNews: 'No news.',
    sentToDepartments: 'Departments',
    attachmentsLabel: 'Attachments',
    runCommunicationsMigration: 'Run migration_communications.sql in Supabase first.',
    archivedNews: 'Archived',
    newsArchived: 'News archived.',
    newsUnarchived: 'News restored.',
    checklistLogNeedsAttention: 'Needs review',
    markAsReviewed: 'Mark as reviewed',
    logMarkedReviewed: 'Log marked as reviewed.',
    reviewedLabel: 'Reviewed',
    checklistLogsMeta: 'Completed checklist history',
    checklistNotesPlaceholder: 'Write any notes before submitting.',
    checklistProgress: '{done} of {total} complete',
    checklists: 'Checklists',
    courses: 'Courses',
    choose: 'Choose',
    customFields: 'Custom fields',
    closeCase: 'Close case',
    completeChecklist: 'Complete checklist',
    completed: 'Completed',
    completedCourses: 'Completed courses',
    completedCoursesMeta: 'Passed course history',
    confirmRead: 'I confirm',
    consequence: 'Consequence',
    contentGroups: 'Content groups',
    correctiveActionPlaceholder: 'Describe what must be done to fix this.',
    correctiveActionRequired: 'Add what must be done for every item outside expected values.',
    createAccount: 'Create account',
    createOne: 'Create one',
    deleteChecklist: 'Delete checklist',
    deleteChecklistConfirm: 'Delete this checklist?',
    deleteChecklistLogConfirm: 'Delete this checklist log permanently? This cannot be undone.',
    deleteCourse: 'Delete course',
    deleteCourseConfirm: 'Delete this course?',
    deleteLog: 'Delete log',
    deleteProcedure: 'Delete procedure',
    deleteReportLogConfirm: 'Delete this report log permanently? This cannot be undone.',
    description: 'Description',
    documents: 'Documents',
    dragReorder: 'Drag to reorder',
    draft: 'draft',
    draftSaved: 'Draft saved.',
    edit: 'Edit',
    editChecklist: 'Edit checklist',
    editCourse: 'Edit course',
    editProcedure: 'Edit procedure',
    editReportForms: 'Edit report forms',
    editReportFormsHint: 'Manage incident, hazard, suggestion, and varsling form fields',
    email: 'Email',
    emailUsernamePasswordRequired: 'Email, username and password are required.',
    enterLogin: 'Enter your username or email and password.',
    entries: 'Entries',
    error: 'Error',
    checklistEmptySubmit: 'Fill in at least one item before submitting.',
    checklistPartialConfirm: '{done} of {total} items are filled in. Submit anyway?',
    invalidCredentials: 'Invalid username or password.',
    anonCaseCodeTitle: 'Your case code',
    anonCaseCodeIntro: 'Write this code down or print it. It is the ONLY way to follow your case and read answers. It cannot be recovered — not even by us.',
    anonCaseCodeConfirm: 'I have saved my code',
    anonCaseCodeCopy: 'Copy code',
    anonCaseCodeDownload: 'Download as file',
    anonCaseSubmitted: 'Your report was submitted anonymously.',
    followCase: 'Follow a case',
    followCaseHint: 'Enter your case code to see the status and messages.',
    caseCode: 'Case code',
    caseNotFound: 'No case found for that code. Check for typos.',
    caseStatusBoard: 'Case board',
    caseStatusBoardHint: 'A dot means new answers. Enter your code above to find your case — the check happens on your device.',
    caseThread: 'Messages',
    caseReplyPlaceholder: 'Write a message to the handlers...',
    caseReplySent: 'Message sent.',
    caseNoMessages: 'No messages yet. Check back in a few days.',
    anonymousCase: 'Anonymous case',
    handlerReply: 'Reply to reporter',
    handlerReplySent: 'Reply sent. The reporter sees it when they check their case.',
    fileAnonymously: 'File anonymously',
    anonLeaveAppConfirm: 'To stay truly anonymous you must file from the public page, which is not linked to your account. Open it now?',
    back: 'Back',
    search: 'Search',
    copied: 'Copied',
    actions: 'Actions',
    newAction: 'New action',
    editAction: 'Edit action',
    actionTitleLabel: 'What must be done?',
    dueDate: 'Due date',
    overdue: 'Overdue',
    done: 'Done',
    cancelledStatus: 'Cancelled',
    markDone: 'Mark as done',
    myActions: 'My actions',
    noActions: 'No actions yet. Create one, or spawn them from reports and checklist deviations.',
    noMyActions: 'No open actions assigned to you.',
    createAction: 'Create action',
    actionSaved: 'Action saved.',
    actionsMigrationHint: 'Run migration_actions.sql in Supabase to enable actions.',
    openActions: 'Open actions',
    doneActions: 'Completed',
    sourceIncident: 'From report',
    sourceChecklistRun: 'From checklist',
    sourceRisk: 'From risk',
    unassigned: 'Unassigned',
    riskAssessment: 'Risk assessment',
    newRisk: 'New risk',
    editRisk: 'Edit risk',
    riskTitleLabel: 'What can go wrong?',
    probability: 'Probability',
    existingMeasures: 'Existing measures',
    riskScore: 'Risk score',
    riskScaleHint: '1 = very low · 5 = very high',
    noRisks: 'No risks assessed yet.',
    risksMigrationHint: 'Run migration_risk_assessments.sql in Supabase to enable risk assessment.',
    riskSaved: 'Risk saved.',
    activeStatus: 'Active',
    highestRisks: 'Highest risks',
    allDepartments: 'All departments',
    confirmReadBtn: 'I have read and understood',
    procedureConfirmed: 'You have confirmed this version.',
    confirmedRead: 'Confirmation saved.',
    readConfirmations: 'Read confirmations',
    readConfirmationsHint: 'Who has confirmed the current version.',
    procedureReadsMigrationHint: 'Run migration_procedure_reads.sql in Supabase to enable read confirmations.',
    notConfirmed: 'Not confirmed',
    versionLabel: 'Version',
    pushNotifications: 'Push notifications',
    enablePush: 'Enable push notifications',
    disablePush: 'Disable push notifications',
    pushHint: 'Get a notification on this device for news, new checklists and actions assigned to you.',
    pushEnabled: 'Push notifications enabled.',
    pushDisabled: 'Push notifications disabled.',
    pushDenied: 'Notification permission was denied.',
    pushUnsupported: 'This browser does not support push notifications.',
    pushNotConfigured: 'Push is not configured yet (missing VAPID key).',
    pushMigrationHint: 'Run migration_push_subscriptions.sql in Supabase to enable push.',
    newActionAssigned: 'New action assigned to you',
    newChecklistAssigned: 'New checklist assigned to you',
    recurrence: 'Recurrence',
    freqNone: 'None (daily quota only)',
    freqWeekly: 'Weekly',
    freqMonthly: 'Monthly',
    thisWeek: 'This week',
    thisMonth: 'This month',
    qrPoster: 'QR poster',
    qrPosterLead: 'Scan to file a confidential report — anonymously if you choose. No login needed.',
    qrPosterFoot: 'Already filed? Follow your case with your case code on the same page.',
    printPoster: 'Print',
    showEmail: 'Show email',
    emailAccessLogged: 'Access is logged',
    emailAccessLog: 'Email access log',
    noEmailReveals: 'No email reveals yet.',
    allUsersLabel: 'all users',
    everyChecklistHitQuota: 'Every checklist hit its daily quota in this range.',
    executiveAnalytics: 'Executive Analytics',
    field: 'Field {n}',
    fields: 'fields',
    finalReport: 'Final report',
    fixed: 'Fixed',
    fixOnNo: 'No requires corrective action',
    coreReportFields: 'These forms keep their core reporting fields. Add custom fields below.',
    groups: 'Groups',
    haveAccount: 'Have an account? Sign in',
    icon: 'Icon',
    incident: 'Incident',
    incidentCategories: 'Incident categories',
    incidentReports: 'Incident reports',
    suggestion: 'Suggestion',
    inProgress: 'In progress',
    items: '{count} items',
    language: 'Language',
    languageSaved: 'Language updated.',
    label: 'Label',
    itemImage: 'Image',
    latestScore: 'Latest: {score}% {status} - {date}',
    learnerMustTick: 'The learner must tick this to earn the points.',
    locationOptional: 'Location (optional)',
    location: 'Location',
    logDeleted: 'Log deleted.',
    logs: 'Archive',
    logsMeta: 'Archived checklists, reports, procedures, and courses',
    missingItems: '{count} missing',
    missing: 'Missing',
    name: 'Name',
    nameRequired: 'Name is required.',
    needAccount: 'Need an account? ',
    newChecklist: 'New checklist',
    newCourse: 'New course',
    newGroup: 'New group',
    newProcedure: 'New procedure',
    noChecklistLogs: 'No checklist logs yet.',
    noChecklists: 'No checklists yet',
    noChecklistsToDo: 'No checklists to do',
    noChecklistsToDoHint: 'Assigned checklists you still need to complete will appear here.',
    noChecklistsMatchFilters: 'No checklists match these filters.',
    noCompletedCourses: 'No completed courses yet.',
    noCompletedCoursesMatchFilters: 'No completed courses match these filters.',
    noChecklistsHint: 'Tap + to build one.',
    noArchivedReportsYet: 'No archived reports yet.',
    noChangeRequestsYet: 'No change requests yet.',
    noCoursesAvailable: 'No courses available.',
    noCoursesYetAddOne: 'No courses yet. Use + to add one.',
    noDocuments: 'No documents',
    noDocumentsHint: 'Nothing shared here yet.',
    noEntries: 'No entries yet',
    noEntriesHint: 'Submissions from users appear here.',
    noIncidents: 'No incidents reported',
    noIncidentsHint: 'Reports submitted by users land here.',
    noGroupFound: 'No {kind} group found.',
    noProfileFound: 'Signed in, but no profile was found for this account.',
    noReportsMatchFilters: 'No reports match these filters.',
    noProcedures: 'No procedures yet',
    noProceduresHint: 'Create readable procedures users can follow.',
    procedureStep: 'Step',
    procedureSteps: 'steps',
    createdBy: 'Created by',
    noReports: 'No reports',
    noReportsHint: 'Incidents you report will appear here.',
    noLabel: 'No',
    noActionsLoggedYet: 'No actions logged yet.',
    noHiddenReports: 'No hidden reports.',
    noAttempts: 'No attempts',
    noAttemptsYet: 'No attempts yet.',
    noVisibleChanges: 'No visible changes',
    noVisibleUsers: 'No visible users found.',
    notAssigned: 'Not assigned',
    notPassedNeeded: 'Not passed - {threshold}% needed',
    notes: 'Notes',
    nothingChanged: 'Nothing changed.',
    open: 'Open',
    openBacklog: 'Open backlog',
    openWithConsequences: 'Open with consequences',
    optionsComma: 'Options (comma-separated)',
    outcomePlaceholder: 'Outcome and corrective actions taken...',
    other: 'Other',
    outsideExpected: 'Needs action',
    removeImage: 'Remove image',
    profileImage: 'Profile image',
    uploadImage: 'Upload image',
    chooseFile: 'Choose file',
    noFileChosen: 'No file chosen',
    filesChosenCount: '{count} files chosen',
    orChooseLogo: 'Or choose a logo',
    password: 'Password',
    photosOptional: 'Pictures (optional)',
    printInspectionReport: 'Print inspection report',
    processing: 'Processing',
    profileChange: 'Profile change',
    profileChangeHint: 'Changes are sent to admins for approval, like account verification.',
    profilePendingChange: 'You already have a pending profile change. Wait for an admin to approve or reject it before sending another one.',
    procedures: 'Procedures',
    procedureSubmissions: 'Procedure submissions',
    questions: 'questions',
    published: 'published',
    publish: 'Publish',
    rejected: 'Rejected',
    reportAnonymously: 'Report anonymously',
    reportFormMigrationNeeded: 'Saved on this device. Run the report form SQL migration to share it with everyone.',
    reportFormSaved: 'Report form saved.',
    reportIncident: 'Report incident',
    reportHidden: 'Report hidden.',
    reportRestored: 'Report restored.',
    reports: 'Reports',
    reportLogsMeta: 'Archived reports, hazards, and suggestions',
    reportTypeHazards: 'Hazards',
    reportTypeIncidents: 'Incidents',
    reportTypeSuggestions: 'Suggestions',
    reportSuggestion: 'Send suggestion',
    saveReportForm: 'Save report form',
    reportsByWorstConsequence: 'Reports by worst consequence',
    reportsByRootCause: 'Reports by root cause',
    filteredBy: 'Filtered by',
    clearFilter: 'Clear filter',
    resetFilters: 'Reset filters',
    allRootCauses: 'All root causes',
    filterByRootCause: 'Filter by root cause',
    reportsClosedInPeriod: 'Reports closed',
    reportsClosedToday: 'Reports closed today',
    reportsInPeriod: 'Reported incidents',
    reportsToday: 'Reports today',
    requestChange: 'Request change',
    reviewTryAgain: 'Review the material and try again.',
    required: 'Required',
    resolved: 'Resolved',
    returnStart: 'Return to start',
    rootCause: 'Root cause',
    saveAssignment: 'Save assignment',
    saveChecklist: 'Save checklist',
    saveDraft: 'Save draft',
    saveFinalReport: 'Save final report',
    score: 'Score',
    searchUsers: 'Search users',
    selectTypeCheckbox: 'Checkbox',
    selectTypeCheckboxes: 'Checkboxes (multiple)',
    selectTypeDropdown: 'Dropdown',
    selectTypeLong: 'Long text',
    selectTypeNumber: 'Number',
    selectTypeShort: 'Short text',
    selectTypeSlider: 'Slider',
    sliderLeft: 'Left label',
    sliderMax: 'Max',
    sliderMin: 'Min',
    sliderRight: 'Right label',
    settings: 'Settings',
    showAllKpis: 'Show all KPIs',
    signIn: 'Sign in',
    signInContinue: 'Sign in to continue.',
    signInTools: 'Sign in to your workplace tools.',
    signOut: 'Sign out',
    signedInAs: 'Signed in as {name}.',
    submit: 'Submit',
    submitReport: 'Submit report',
    submitted: 'Submitted',
    submittedAnswers: 'Submitted answers',
    submittedPhoto: 'Submitted picture',
    suggestionText: 'What would you like to suggest?',
    superAdmins: 'Super admins',
    roleUser: 'User',
    roleVerneombud: 'Safety rep',
    roleAdmin: 'Admin',
    roleSuperuser: 'Superuser',
    roleSysadmin: 'Sys-admin',
    safetyReps: 'Safety reps',
    superusers: 'Superusers',
    sysadmins: 'Sys-admins',
    varsling: 'Varsling',
    varslingDesc: 'Confidential report for serious matters. Handled by a few selected people.',
    fileVarsling: 'File a varsling',
    varslingTitle: 'What is this about?',
    varslingDetails: 'Describe what happened',
    varslingSubmitted: 'Varsling submitted. It will be handled confidentially.',
    appendUpdate: 'Append update',
    appendUpdateEmpty: 'Add a note or picture first.',
    appendUpdateHint: 'Adds a new audit-log entry without changing the original report.',
    appendUpdatePlaceholder: 'Add a fresh note, follow-up detail, or corrective update...',
    attachmentsAdded: 'Pictures added.',
    caseTeam: 'Case team',
    escalationReasonPlaceholder: 'Explain why this closed case should be reviewed again...',
    hiddenArchivedReports: 'Hidden / Archived Reports',
    hide: 'Hide',
    incidentEscalated: 'Incident re-opened and escalated.',
    reopenEscalate: 'Re-open & Escalate',
    reopenEscalateHint: 'Use this if you believe the case was dismissed or handled incorrectly.',
    unhide: 'Unhide',
    updateAppended: 'Update appended.',
    workerAddendum: 'Worker addendum',
    you: 'You',
    varslingAccess: 'Varsling access',
    varslingAccessGranted: 'Varsling access granted.',
    varslingAccessRevoked: 'Varsling access revoked.',
    noVarslinger: 'No varslinger.',
    submitAnonymously: 'Submit anonymously',
    assigned: 'Assigned',
    assignedToComplete: 'Assigned to complete',
    reason: 'Reason',
    reasonRequired: 'Add a reason first.',
    revoke: 'Revoke',
    suspend: 'Suspend',
    suspended: 'Suspended',
    unsuspend: 'Unsuspend',
    accountSuspended: 'Account suspended',
    accountSuspendedBody: 'Your account is under sys-admin review and is temporarily locked. Another sys-admin must resolve the case.',
    sysadminReviewTitle: 'Sys-admin action requires review',
    sysadminReviewBody: 'Give a clear reason for this sys-admin action.',
    emergencyTitle: '🚨 Sys-admin emergency',
    emergencyBody: 'A sys-admin opened a case against another sys-admin. Review and decide.',
    upholdAction: 'Uphold',
    revertAction: 'Revert (keep initiator suspended)',
    bothSuspended: 'Both accounts have been suspended pending review.',
    status: 'Status',
    resolution: 'Resolution / notes',
    save: 'Save',
    deleteAction: 'Delete',
    saved: 'Saved.',
    selectAll: 'Select all',
    selectAssignedUser: 'Select at least one assigned user.',
    selectUnassignedUser: 'Select at least one unassigned user.',
    title: 'Title',
    toggleDarkMode: 'Toggle dark mode',
    today: 'Today',
    totalAdmins: 'Total admins',
    totalUsers: 'Total users',
    topType: 'Top type',
    total: 'total',
    type: 'Type',
    uniqueUsers: 'Unique users',
    yes: 'Yes',
    no: 'No',
    username: 'Username',
    usernameEmailRequired: 'Username and email are required.',
    usernameTaken: 'That username is taken.',
    upload: 'Upload',
    uploadDocument: 'Upload a document',
    users: 'Users',
    usernameOrEmail: 'Username or email',
    userEmailNotFound: 'No account found for that username.',
    waitingApproval: 'Waiting for approval',
    whatHappened: 'What happened?',
    whatHappenedTitle: 'What happened',
    whatMustBeDone: 'What must be done to fix this?',
    whatNeedsFixing: 'What needs fixing?',
    whenHappened: 'When did it happen?',
    withinExpected: 'OK',
    workerApp: 'Worker app',
    workerAppMeta: 'Checklists, reports, and documents',
    questionImage: 'Checklist picture',
    needsActionCount: '{count} need action',
    yesLabel: 'Yes',
    yourRecentReports: 'Your recent reports',
    yourReport: 'Your report',
    yourDetails: 'Your details',
    dailyQuota: 'Daily quota',
    quotaDays: 'Counts on',
    kpiOverview: 'Overview',
    kpiIncidents: 'Incidents',
    kpiChecklists: 'Checklists',
    kpiAdminPerformance: 'Admin performance',
    rightNow: 'Right now',
    inThisPeriod: 'In this period',
    dueToday: 'Due today',
    pendingToday: 'Pending today',
    activeChecklists: 'Active checklists',
    kpiWorkload: 'Workload',
    reportsFiled: 'Reports filed',
    reportsResolved: 'Reports resolved',
    checklistsDone: 'Checklists done',
    coursesPassed: 'Courses passed',
    proceduresFiled: 'Procedures filed',
    documentsAdded: 'Documents added',
    newMembers: 'New members',
    varslingerFiled: 'Whistleblowing',
    reportsByType: 'Reports by type',
    latestActivity: 'Latest activity',
    noActivityYet: 'No activity in this period yet.',
    periodToday: 'Day',
    periodWeek: 'Week',
    periodMonth: 'Month',
    periodCustom: 'Custom',
    actIncidentReported: 'Incident reported',
    actHazardReported: 'Hazard reported',
    actSuggestionSent: 'Suggestion sent',
    actReportResolved: 'Report resolved',
    actChecklistDone: 'Checklist completed',
    actCoursePassed: 'Course passed',
    actCourseAttempt: 'Course attempt',
    actProcedureFiled: 'Procedure submitted',
    actDocumentAdded: 'Document added',
    actMemberJoined: 'New member joined',
    actVarslingFiled: 'Whistleblowing filed',
    confidential: 'Confidential',
    assignedReports: 'Assigned reports',
    completedTotal: 'Completed total',
    completionRate: 'Completion rate',
    completedToday: 'Completed today',
    noAdminsFound: 'No admins found',
    createOrPromoteAdmin: 'Create or promote an admin first.',
    seeWhich: 'See which',
    expectedNote: 'Expected = daily quota × qualifying days. Match a gap to whoever was on shift.',
    latestChecklists: 'Latest checklists',
    noRecentChecklists: 'No checklist submissions in this period.',
    expectedVsReceived: 'Expected vs received',
    dailyExpected: 'Daily expected',
    expectedInRange: 'Expected in selected range',
    receivedLabel: 'Received',
    perDayShort: 'day',
    more: 'more',
    showLess: 'Show less',
    tapToSeeFiled: 'Tap to see when each was filed ▾',
    whenFiled: 'When filed',
    allAdmins: 'All admins',
    completionByAdmin: 'Completion by admin',
    hiddenGroups: 'Hidden groups',
    noHiddenGroups: 'No hidden groups.',
    restore: 'Restore',
    permanentDelete: 'Permanently delete',
    requestDelete: 'Request delete',
    approveDelete: 'Approve delete',
    deleteReason: 'Reason for deletion',
    deleteRequested: 'Delete request submitted.',
    deleteApproved: 'Delete request approved.',
    groupDeleted: 'Group permanently deleted.',
    pendingDeleteRequest: 'Pending delete request',
    pendingDeleteRequests: 'Pending delete requests',
    hideGroup: 'Hide group',
    groupHidden: 'Group hidden.',
    groupRestored: 'Group restored.',
    moveToTrash: 'Move to trash',
    confirmMoveToTrash: 'Move to trash:',
    movedToTrash: 'Moved to trash.',
    trashBin: 'Trash bin',
    trashRetentionHint: 'Deleted items stay here for 30 days before permanent deletion.',
    trashEmpty: 'Trash is empty.',
    trashDayLeft: '1 day left',
    trashDaysLeft: '{count} days left',
    deleteNow: 'Delete now',
    confirmDeleteTrashNow: 'Permanently delete all items in trash now? This cannot be undone.',
    trashDeleted: 'Trash deleted.',
    recover: 'Recover',
    restoredFromTrash: 'Recovered from trash.',
    runTrashMigration: 'Run migration_trash_soft_delete.sql to enable the trash bin.',
    departments: 'Departments',
    departmentName: 'Department name',
    departmentsAdminHint: 'Manage departments and assign workers below.',
    visibleToDepartments: 'Visible to departments',
    departmentsHint: 'If none are checked, this item is visible to everyone.',
    generalDepartmentHint: 'Default department. Every user belongs to this department.',
    departmentMembershipHint: 'Select which users belong to this department.',
    import: 'Import',
    importTitle: 'Import into this group',
    importHint: 'Pick existing items from other groups. They stay linked — editing one updates it everywhere.',
    selectItemsToImport: 'Select items to import',
    noItemsToImport: 'No items in other groups to import.',
    itemImported: 'Imported into this group.',
    itemsImported: 'Imported {count} item(s) into this group.',
    linked: 'Linked',
    removeFromGroup: 'Remove from group',
    removedFromGroup: 'Removed from this group.',
    importSelected: 'Import selected',
    fromGroupLabel: 'From {group}',
    waitingVerification: 'Waiting for verification',
    waitingVerificationBody: 'Your account is set up, but an admin needs to approve it before you can use the app. You\'ll get in as soon as they do.',
    checkAgain: 'Check again',
    fullName: 'Full name',
    noAccountsOrChanges: 'No accounts or profile changes waiting.',
    noAdminAccess: 'No admin access',
    noAdminAccessBody: 'You\'re signed in as {name}, but this is the admin console. An admin must grant access.',
    assignedToDo: 'Assigned to do',
    assign: 'Assign',
    nothingAssignedYet: 'Nothing assigned yet.',
    latestSubmitted: 'Latest submitted',
    noSubmissionsYet: 'No submissions yet.',
    noSubmissions: 'no submissions',
    lastSubmitted: 'last',
    addMember: 'Add member',
    removeMember: 'Remove',
    confirmDeleteDepartment: 'Delete this department? This will unassign all members.',
    selectUserToAssign: 'Select user to assign...',
    defaultDepartment: 'Default',
    deleteDepartment: 'Delete Department',
    folderVisibility: 'Folder Visibility',
    saveVisibility: 'Save visibility',
    visibilityTitle: 'Visibility: {name}',
    visibility: 'Visibility',
  },
  no: {
    accounts: 'Kontoer',
    addTitleAndField: 'Legg til en tittel og minst ett felt.',
    addTitleAndItem: 'Legg til en tittel og minst ett element.',
    assignmentRemoved: 'Tildeling fjernet',
    belowQuotaAcross: '{missed} manglende over {days} dager',
    caseResolved: 'Sak løst',
    changeRequestSent: 'Endringsforespørsel sendt',
    checklistTypes: 'Sjekklistetyper',
    chooseCourseFirst: 'Velg et kurs først.',
    chooseFileFirst: 'Velg en fil først.',
    chooseVisibleAssignee: 'Velg en synlig mottaker.',
    courseAssigned: 'Kurs tildelt',
    currentWorkspace: 'Nåværende arbeidsområde',
    deleted: 'Slettet',
    emailUsernameTempPasswordRequired: 'E-post, brukernavn og midlertidig passord er påkrevd.',
    nameEmailUsernamePasswordRequired: 'Navn, e-post, brukernavn og passord er påkrevd.',
    nameEmailUsernameTempPasswordRequired: 'Navn, e-post, brukernavn og midlertidig passord er påkrevd.',
    exampleKitchen: 'f.eks. Kjøkken',
    excelLibraryLoadFailed: 'Kunne ikke laste regnearkbiblioteket.',
    expectedGot: 'forventet {expected}, fikk {got}',
    expectedIssues: 'Forventede avvik',
    finalReportSaved: 'Sluttrapport lagret',
    finalReportVisibleHint: 'Sluttrapporten er synlig for melderen når den lagres.',
    folders: 'Mapper',
    groupAdded: 'Gruppe lagt til',
    importBelowRoleOnly: 'Du kan bare importere brukere under din egen rolle.',
    manageCourses: 'Administrer kurs',
    managementPanel: 'Administrasjonspanel',
    medianResolve: 'Median løstid',
    migrationChecklistLogsMissing: 'Kjør sjekkliste-loggmigreringen for å aktivere dette.',
    migrationCoursesMissing: 'Kjør kursmigreringen for å aktivere dette.',
    missed: 'Manglende',
    nothingToExport: 'Ingenting å eksportere.',
    notificationPreferenceSaved: 'Varslingsinnstilling lagret',
    oldestOpen: 'Eldste åpne',
    passwordMinLength: 'Passordet må være minst 6 tegn.',
    profileChangeApproved: 'Profilendring godkjent',
    profileChangeRejected: 'Profilendring avvist',
    refresh: 'Oppdater',
    reported: 'Rapportert',
    resolutionRate: 'Loseprosent',
    serious: 'Alvorlige',
    shortCount: '{count} for fa',
    spreadsheetEmpty: 'Regnearket er tomt.',
    uploaded: 'Lastet opp',
    userAddedTempPassword: 'Bruker lagt til med et midlertidig passord.',
    userCreatedNoId: 'Bruker opprettet, men ingen ID ble returnert.',
    workspace: 'Arbeidsomrade',
    access: 'Tilgang',
    accountsAccess: 'Kontoer og tilgang',
    actionLog: 'Handlingslogg',
    add: 'Legg til',
    addActionNote: 'Legg til handling / notat',
    addField: '+ Legg til felt',
    addProcedureStep: '+ Legg til prosedyresteg',
    addGroup: 'Legg til gruppe',
    addItem: '+ Legg til punkt',
    addToLog: 'Legg til i logg',
    addUser: '+ Legg til bruker',
    adminConsole: 'Administrasjon',
    adminConsoleMeta: 'Administrer grupper, hendelser og tilgang',
    all: 'Alle',
    allChecklists: 'Alle sjekklister',
    apply: 'Bruk',
    allTypes: 'Alle typer',
    anonymous: 'Anonym',
    admins: 'Administratorer',
    archive: 'Arkiver',
    archived: 'Arkivert',
    archivedReports: 'Arkiverte rapporter',
    checklistLogs: 'Sjekklistelogger',
    dateFrom: 'Fra',
    dateUntil: 'Til',
    dashboardPeriod: 'Dashboardperiode',
    periodPrompt: 'Bruk YYYY-MM-DD eller YYYY-MM-DD..YYYY-MM-DD.',
    invalidPeriod: 'Bruk YYYY-MM-DD eller YYYY-MM-DD..YYYY-MM-DD.',
    exportExcel: 'Eksporter til Excel',
    importExcel: 'Importer Excel',
    expected: 'Forventet',
    expectedChecked: 'Forventet avkrysning',
    expectedMax: 'Forventet maks',
    expectedMin: 'Forventet min',
    expectedValues: 'Forventede verdier',
    filterByType: 'Rapporttype',
    filterByUser: 'Bruker',
    hazard: 'Fare',
    hazardConsequences: 'Hva kan det føre til?',
    notifications: 'Varslinger',
    notifyAll: 'Alle rapporter',
    notifyOff: 'Av',
    notifyRed: 'Kun nødstilfeller',
    reportHazard: 'Rapporter fare',
    roundNearest: 'Nærmeste',
    roundUp: 'Rund opp',
    sliderRound: 'Visning',
    unarchive: 'Gjenopprett',
    whatHazard: 'Hva er faren?',
    whenSpotted: 'Når oppdaget du den?',
    anyAssignee: 'Alle tildelte',
    anyCourse: 'Alle kurs',
    anyForm: 'Alle skjemaer',
    filterByForm: 'Skjema',
    form: 'Skjema',
    procedureLogsMeta: 'Innsendte prosedyreskjemaer',
    noProcedureSubmissions: 'Ingen prosedyreinnsendinger ennå',
    noProcedureSubmissionsMatchFilters: 'Ingen prosedyreinnsendinger samsvarer med filtrene',
    anyReporter: 'Alle',
    filterByAssignee: 'Tildelt admin',
    filterByReporter: 'Rapportert av',
    unassigned: 'Ikke tildelt',
    assignedTo: 'Tildelt til',
    inProgressAssignedToYou: 'Pågår og tildelt deg',
    attachments: 'Bilder',
    changeRequests: 'Endringsforespørsler',
    checklist: 'Sjekkliste',
    course: 'Kurs',
    procedure: 'Prosedyre',
    checklistComplete: 'Sjekkliste sendt inn.',
    checklistTypeCheck: 'Avkryssing',
    checklistTypeChoice: 'Flervalg',
    checklistTypeText: 'Tekst',
    checklistsCompletedInPeriod: 'Fullførte sjekklister',
    checklistsCompletedToday: 'Fullførte sjekklister i dag',
    checklistsToDo: 'Sjekklister å gjøre',
    todaysChecklists: 'Dagens sjekklister',
    greetingMorning: 'God morgen',
    greetingAfternoon: 'God ettermiddag',
    greetingEvening: 'God kveld',
    doneOfTotalToday: '{done} av {total} fullført i dag',
    allChecklistsDone: 'Alle sjekklister er fullført i dag',
    nothingScheduledToday: 'Ingen sjekklister planlagt i dag',
    quickActions: 'Hurtigvalg',
    yourActivity: 'Din aktivitet',
    myReports: 'Mine rapporter',
    communications: 'Kommunikasjon',
    news: 'Nyheter',
    safetyNews: 'Sikkerhetsnyhet',
    safetyNewsToggle: 'Merk som sikkerhetsnyhet',
    noSafetyNews: 'Ingen sikkerhetsnyheter.',
    newNews: 'Ny nyhet',
    commTitle: 'Tittel',
    commBody: 'Melding',
    commImage: 'Forsidebilde (valgfritt)',
    commAttachments: 'Vedlegg (valgfritt)',
    broadcastAudience: 'Mottakere',
    audienceEveryone: 'Alle',
    audienceDepartments: 'Bestemte avdelinger',
    selectDepartments: 'Velg avdelinger',
    pickAtLeastOneDepartment: 'Velg minst en avdeling.',
    messageBodyRequired: 'Legg til en tittel eller melding først.',
    send: 'Send',
    sending: 'Sender…',
    communicationSent: 'Sendt.',
    readReceipts: 'Lesebekreftelser',
    readByCount: '{read} av {total} lest',
    readBy: 'Lest av',
    notYetRead: 'Ikke lest ennå',
    unread: 'Ulest',
    noCommunications: 'Ingen kommunikasjon ennå.',
    noCommunicationsHint: 'Nyheter fra arbeidsplassen din vises her.',
    noNews: 'Ingen nyheter.',
    sentToDepartments: 'Avdelinger',
    attachmentsLabel: 'Vedlegg',
    runCommunicationsMigration: 'Kjør migration_communications.sql i Supabase først.',
    archivedNews: 'Arkivert',
    newsArchived: 'Nyhet arkivert.',
    newsUnarchived: 'Nyhet gjenopprettet.',
    checklistLogNeedsAttention: 'Må ses gjennom',
    markAsReviewed: 'Marker som gjennomgått',
    logMarkedReviewed: 'Logg markert som gjennomgått.',
    reviewedLabel: 'Gjennomgått',
    checklistLogsMeta: 'Historikk for innsendte sjekklister',
    checklistNotesPlaceholder: 'Skriv eventuelle notater for innsending.',
    checklistProgress: '{done} av {total} fullført',
    checklists: 'Sjekklister',
    courses: 'Kurs',
    choose: 'Velg',
    customFields: 'Egendefinerte felt',
    closeCase: 'Lukk sak',
    completeChecklist: 'Fullfør sjekkliste',
    completed: 'Fullført',
    completedCourses: 'Fullførte kurs',
    completedCoursesMeta: 'Historikk for beståtte kurs',
    consequence: 'Konsekvens',
    correctiveActionPlaceholder: 'Beskriv hva som må gjøres for å rette dette.',
    correctiveActionRequired: 'Fyll inn hva som må gjøres for alle punkter utenfor forventede verdier.',
    createAccount: 'Opprett konto',
    createOne: 'Opprett en',
    deleteChecklist: 'Slett sjekkliste',
    deleteLog: 'Slett logg',
    deleteProcedure: 'Slett prosedyre',
    description: 'Beskrivelse',
    documents: 'Dokumenter',
    dragReorder: 'Dra for å endre rekkefølge',
    draft: 'utkast',
    draftSaved: 'Utkast lagret.',
    edit: 'Rediger',
    editChecklist: 'Rediger sjekkliste',
    editReportForms: 'Rediger rapportskjemaer',
    editReportFormsHint: 'Administrer felt for hendelse, fare, forslag og varsling',
    editProcedure: 'Rediger prosedyre',
    email: 'E-post',
    enterLogin: 'Skriv inn brukernavn eller e-post og passord.',
    entries: 'Innsendinger',
    error: 'Feil',
    checklistEmptySubmit: 'Fyll ut minst ett punkt før du sender inn.',
    checklistPartialConfirm: '{done} av {total} punkter er fylt ut. Sende inn likevel?',
    invalidCredentials: 'Ugyldig brukernavn eller passord.',
    anonCaseCodeTitle: 'Din sakskode',
    anonCaseCodeIntro: 'Skriv ned eller skriv ut denne koden. Den er den ENESTE måten å følge saken og lese svar på. Den kan ikke gjenopprettes — heller ikke av oss.',
    anonCaseCodeConfirm: 'Jeg har lagret koden min',
    anonCaseCodeCopy: 'Kopier kode',
    anonCaseCodeDownload: 'Last ned som fil',
    anonCaseSubmitted: 'Varselet ble sendt anonymt.',
    followCase: 'Følg en sak',
    followCaseHint: 'Skriv inn sakskoden for å se status og meldinger.',
    caseCode: 'Sakskode',
    caseNotFound: 'Fant ingen sak med den koden. Sjekk for skrivefeil.',
    caseStatusBoard: 'Sakstavle',
    caseStatusBoardHint: 'En prikk betyr nye svar. Skriv inn koden over for å finne saken din — sjekken skjer på din enhet.',
    caseThread: 'Meldinger',
    caseReplyPlaceholder: 'Skriv en melding til saksbehandlerne...',
    caseReplySent: 'Melding sendt.',
    caseNoMessages: 'Ingen meldinger ennå. Sjekk igjen om noen dager.',
    anonymousCase: 'Anonym sak',
    handlerReply: 'Svar til melderen',
    handlerReplySent: 'Svar sendt. Melderen ser det når de sjekker saken sin.',
    fileAnonymously: 'Send anonymt',
    anonLeaveAppConfirm: 'For å være helt anonym må du melde fra den offentlige siden, som ikke er knyttet til kontoen din. Åpne den nå?',
    back: 'Tilbake',
    search: 'Søk',
    copied: 'Kopiert',
    actions: 'Tiltak',
    newAction: 'Nytt tiltak',
    editAction: 'Rediger tiltak',
    actionTitleLabel: 'Hva må gjøres?',
    dueDate: 'Frist',
    overdue: 'Forfalt',
    done: 'Utført',
    cancelledStatus: 'Avbrutt',
    markDone: 'Merk som utført',
    myActions: 'Mine tiltak',
    noActions: 'Ingen tiltak ennå. Opprett ett, eller lag dem fra rapporter og sjekklisteavvik.',
    noMyActions: 'Ingen åpne tiltak tildelt deg.',
    createAction: 'Opprett tiltak',
    actionSaved: 'Tiltak lagret.',
    actionsMigrationHint: 'Kjør migration_actions.sql i Supabase for å aktivere tiltak.',
    openActions: 'Åpne tiltak',
    doneActions: 'Utførte',
    sourceIncident: 'Fra rapport',
    sourceChecklistRun: 'Fra sjekkliste',
    sourceRisk: 'Fra risiko',
    unassigned: 'Ikke tildelt',
    riskAssessment: 'Risikovurdering',
    newRisk: 'Ny risiko',
    editRisk: 'Rediger risiko',
    riskTitleLabel: 'Hva kan gå galt?',
    probability: 'Sannsynlighet',
    existingMeasures: 'Eksisterende tiltak',
    riskScore: 'Risikoverdi',
    riskScaleHint: '1 = svært lav · 5 = svært høy',
    noRisks: 'Ingen risikoer vurdert ennå.',
    risksMigrationHint: 'Kjør migration_risk_assessments.sql i Supabase for å aktivere risikovurdering.',
    riskSaved: 'Risiko lagret.',
    activeStatus: 'Aktiv',
    highestRisks: 'Høyeste risikoer',
    allDepartments: 'Alle avdelinger',
    confirmReadBtn: 'Jeg har lest og forstått',
    procedureConfirmed: 'Du har bekreftet denne versjonen.',
    confirmedRead: 'Bekreftelse lagret.',
    readConfirmations: 'Lesebekreftelser',
    readConfirmationsHint: 'Hvem som har bekreftet gjeldende versjon.',
    procedureReadsMigrationHint: 'Kjør migration_procedure_reads.sql i Supabase for å aktivere lesebekreftelser.',
    notConfirmed: 'Ikke bekreftet',
    versionLabel: 'Versjon',
    pushNotifications: 'Pushvarsler',
    enablePush: 'Slå på pushvarsler',
    disablePush: 'Slå av pushvarsler',
    pushHint: 'Få varsel på denne enheten om nyheter, nye sjekklister og tiltak tildelt deg.',
    pushEnabled: 'Pushvarsler er på.',
    pushDisabled: 'Pushvarsler er av.',
    pushDenied: 'Varseltillatelse ble avslått.',
    pushUnsupported: 'Denne nettleseren støtter ikke pushvarsler.',
    pushNotConfigured: 'Push er ikke konfigurert ennå (mangler VAPID-nøkkel).',
    pushMigrationHint: 'Kjør migration_push_subscriptions.sql i Supabase for å aktivere push.',
    newActionAssigned: 'Nytt tiltak tildelt deg',
    newChecklistAssigned: 'Ny sjekkliste tildelt deg',
    recurrence: 'Gjentakelse',
    freqNone: 'Ingen (kun dagskvote)',
    freqWeekly: 'Ukentlig',
    freqMonthly: 'Månedlig',
    thisWeek: 'Denne uken',
    thisMonth: 'Denne måneden',
    qrPoster: 'QR-plakat',
    qrPosterLead: 'Skann for å varsle konfidensielt — anonymt hvis du vil. Ingen innlogging nødvendig.',
    qrPosterFoot: 'Allerede varslet? Følg saken din med sakskoden på samme side.',
    printPoster: 'Skriv ut',
    showEmail: 'Vis e-post',
    emailAccessLogged: 'Tilgang loggføres',
    emailAccessLog: 'E-posttilgangslogg',
    noEmailReveals: 'Ingen e-postvisninger ennå.',
    allUsersLabel: 'alle brukere',
    field: 'Felt {n}',
    fields: 'felt',
    finalReport: 'Sluttrapport',
    fixed: 'Rettet',
    fixOnNo: 'Nei krever korrigerende tiltak',
    coreReportFields: 'Skjemaene beholder standardfeltene. Legg til egendefinerte felt under.',
    groups: 'Grupper',
    haveAccount: 'Har du konto? Logg inn',
    incident: 'Hendelse',
    incidentCategories: 'Hendelseskategorier',
    incidentReports: 'Hendelsesrapporter',
    suggestion: 'Forslag',
    inProgress: 'Pagar',
    items: '{count} punkter',
    language: 'Språk',
    languageSaved: 'Språk oppdatert.',
    label: 'Etikett',
    itemImage: 'Bilde',
    locationOptional: 'Sted (valgfritt)',
    location: 'Sted',
    logs: 'Arkiv',
    logsMeta: 'Arkiverte sjekklister, rapporter, prosedyrer og kurs',
    missingItems: '{count} mangler',
    missing: 'Mangler',
    name: 'Navn',
    needAccount: 'Trenger du konto? ',
    newChecklist: 'Ny sjekkliste',
    newGroup: 'Ny gruppe',
    newProcedure: 'Ny prosedyre',
    noChecklistLogs: 'Ingen sjekklistelogger ennå.',
    noChecklists: 'Ingen sjekklister ennå',
    noChecklistsToDo: 'Ingen sjekklister å gjøre',
    noChecklistsToDoHint: 'Tildelte sjekklister du må fullføre vises her.',
    noCompletedCourses: 'Ingen fullførte kurs ennå.',
    noChecklistsHint: 'Trykk + for å lage en.',
    noDocuments: 'Ingen dokumenter',
    noDocumentsHint: 'Ingenting er delt her ennå.',
    noEntries: 'Ingen innsendinger ennå',
    noEntriesHint: 'Innsendinger fra brukere vises her.',
    noIncidents: 'Ingen hendelser rapportert',
    noIncidentsHint: 'Rapporter fra brukere vises her.',
    noProcedures: 'Ingen prosedyrer ennå',
    noProceduresHint: 'Lag lesbare prosedyrer brukere kan følge.',
    procedureStep: 'Steg',
    procedureSteps: 'steg',
    createdBy: 'Opprettet av',
    noReports: 'Ingen rapporter',
    noReportsHint: 'Hendelser du rapporterer vises her.',
    noLabel: 'Nei',
    notes: 'Notater',
    open: 'Åpen',
    openWithConsequences: 'Åpen med konsekvenser',
    optionsComma: 'Valg (kommaseparert)',
    other: 'Annet',
    outsideExpected: 'Krever tiltak',
    removeImage: 'Fjern bilde',
    profileImage: 'Profilbilde',
    uploadImage: 'Last opp bilde',
    chooseFile: 'Velg fil',
    noFileChosen: 'Ingen fil valgt',
    filesChosenCount: '{count} filer valgt',
    orChooseLogo: 'Eller velg en logo',
    password: 'Passord',
    photosOptional: 'Bilder (valgfritt)',
    printInspectionReport: 'Skriv ut inspeksjonsrapport',
    processing: 'Behandling',
    procedures: 'Prosedyrer',
    questions: 'spørsmål',
    published: 'publisert',
    publish: 'Publiser',
    reportAnonymously: 'Rapporter anonymt',
    reportFormMigrationNeeded: 'Lagret på denne enheten. Kjør SQL-migreringen for rapportskjemaer for å dele med alle.',
    reportFormSaved: 'Rapportskjema lagret.',
    reportIncident: 'Rapporter hendelse',
    reportHidden: 'Rapport skjult.',
    reportRestored: 'Rapport gjenopprettet.',
    reports: 'Rapporter',
    reportLogsMeta: 'Arkiverte rapporter, farer og forslag',
    reportTypeHazards: 'Farer',
    reportTypeIncidents: 'Hendelser',
    reportTypeSuggestions: 'Forslag',
    reportSuggestion: 'Send forslag',
    saveReportForm: 'Lagre rapportskjema',
    reportsClosedInPeriod: 'Rapporter lukket',
    reportsClosedToday: 'Rapporter lukket i dag',
    reportsInPeriod: 'Rapporterte hendelser',
    reportsToday: 'Rapporter i dag',
    requestChange: 'Be om endring',
    required: 'Påkrevd',
    resolved: 'Løst',
    returnStart: 'Tilbake til start',
    rootCause: 'Rotårsak',
    saveAssignment: 'Lagre tildeling',
    saveChecklist: 'Lagre sjekkliste',
    saveDraft: 'Lagre utkast',
    saveFinalReport: 'Lagre sluttrapport',
    score: 'Poeng',
    searchUsers: 'Søk etter brukere',
    selectTypeCheckbox: 'Avkryssing',
    selectTypeCheckboxes: 'Avkryssing (flere)',
    selectTypeDropdown: 'Nedtrekk',
    selectTypeLong: 'Lang tekst',
    selectTypeNumber: 'Tall',
    selectTypeShort: 'Kort tekst',
    selectTypeSlider: 'Skala',
    sliderLeft: 'Venstre etikett',
    sliderMax: 'Maks',
    sliderMin: 'Min',
    sliderRight: 'Høyre etikett',
    suggestionText: 'Hva vil du foreslå?',
    roleUser: 'Bruker',
    roleVerneombud: 'Verneombud',
    roleAdmin: 'Administrator',
    roleSuperuser: 'Superbruker',
    roleSysadmin: 'Sys-admin',
    safetyReps: 'Verneombud',
    superusers: 'Superbrukere',
    sysadmins: 'Sys-admins',
    varsling: 'Varsling',
    varslingDesc: 'Konfidensiell melding for alvorlige forhold. Behandles av noen få utvalgte.',
    fileVarsling: 'Send en varsling',
    varslingTitle: 'Hva gjelder det?',
    varslingDetails: 'Beskriv hva som har skjedd',
    varslingSubmitted: 'Varsling sendt. Den behandles konfidensielt.',
    appendUpdate: 'Legg til oppdatering',
    appendUpdateEmpty: 'Legg til et notat eller bilde først.',
    appendUpdateHint: 'Legger til en ny loggoppføring uten å endre originalrapporten.',
    appendUpdatePlaceholder: 'Legg til nytt notat, oppfølging eller korrigerende oppdatering...',
    attachmentsAdded: 'Bilder lagt til.',
    caseTeam: 'Saksteam',
    escalationReasonPlaceholder: 'Forklar hvorfor denne lukkede saken bør vurderes på nytt...',
    hiddenArchivedReports: 'Skjulte / arkiverte rapporter',
    hide: 'Skjul',
    incidentEscalated: 'Hendelsen er gjenåpnet og eskalert.',
    reopenEscalate: 'Gjenåpne og eskaler',
    reopenEscalateHint: 'Bruk dette hvis du mener saken ble avvist eller håndtert feil.',
    unhide: 'Vis igjen',
    updateAppended: 'Oppdatering lagt til.',
    workerAddendum: 'Arbeidertillegg',
    you: 'Du',
    varslingAccess: 'Varslingstilgang',
    varslingAccessGranted: 'Varslingstilgang gitt.',
    varslingAccessRevoked: 'Varslingstilgang fjernet.',
    noVarslinger: 'Ingen varslinger.',
    submitAnonymously: 'Send anonymt',
    assigned: 'Tildelt',
    assignedToComplete: 'Tildelt for fullføring',
    reason: 'Begrunnelse',
    reasonRequired: 'Legg til begrunnelse først.',
    revoke: 'Trekk tilbake',
    suspend: 'Suspender',
    suspended: 'Suspendert',
    unsuspend: 'Opphev suspensjon',
    accountSuspended: 'Konto suspendert',
    accountSuspendedBody: 'Kontoen din er under sys-admin-vurdering og er midlertidig låst. En annen sys-admin må avgjøre saken.',
    sysadminReviewTitle: 'Sys-admin-handling krever vurdering',
    sysadminReviewBody: 'Gi en tydelig begrunnelse for denne sys-admin-handlingen.',
    emergencyTitle: '🚨 Sys-admin-nødsituasjon',
    emergencyBody: 'En sys-admin opprettet en sak mot en annen sys-admin. Vurder og bestem.',
    upholdAction: 'Oppretthold',
    revertAction: 'Reverser (hold initiativtaker suspendert)',
    bothSuspended: 'Begge kontoer er suspendert i påvente av vurdering.',
    status: 'Status',
    resolution: 'Løsning / notater',
    save: 'Lagre',
    deleteAction: 'Slett',
    saved: 'Lagret.',
    title: 'Tittel',
    toggleDarkMode: 'Bytt mørk modus',
    totalAdmins: 'Totalt administratorer',
    totalUsers: 'Totalt brukere',
    type: 'Type',
    upload: 'Last opp',
    uploadDocument: 'Last opp dokument',
    users: 'Brukere',
    usernameOrEmail: 'Brukernavn eller e-post',
    userEmailNotFound: 'Fant ingen konto med det brukernavnet.',
    waitingApproval: 'Venter på godkjenning',
    whatHappened: 'Hva skjedde?',
    whatHappenedTitle: 'Hva skjedde',
    whatMustBeDone: 'Hva må gjøres for å rette dette?',
    whatNeedsFixing: 'Hva må rettes?',
    whenHappened: 'Når skjedde det?',
    withinExpected: 'OK',
    workerApp: 'Arbeiderapp',
    workerAppMeta: 'Sjekklister, rapporter og dokumenter',
    questionImage: 'Sjekklistebilde',
    needsActionCount: '{count} krever tiltak',
    yesLabel: 'Ja',
    yourRecentReports: 'Dine siste rapporter',
    yourReport: 'Din rapport',
    yourDetails: 'Dine detaljer',
    dailyQuota: 'Dagskvote',
    quotaDays: 'Telles på',
    kpiOverview: 'Oversikt',
    kpiIncidents: 'Hendelser',
    kpiChecklists: 'Sjekklister',
    kpiAdminPerformance: 'Adminytelse',
    rightNow: 'Akkurat nå',
    inThisPeriod: 'I denne perioden',
    dueToday: 'Forfaller i dag',
    pendingToday: 'Gjenstår i dag',
    activeChecklists: 'Aktive sjekklister',
    kpiWorkload: 'Arbeidsmengde',
    reportsFiled: 'Rapporter mottatt',
    reportsResolved: 'Rapporter løst',
    checklistsDone: 'Sjekklister fullført',
    coursesPassed: 'Kurs bestått',
    proceduresFiled: 'Prosedyrer levert',
    documentsAdded: 'Dokumenter lagt til',
    newMembers: 'Nye medlemmer',
    varslingerFiled: 'Varslinger',
    reportsByType: 'Rapporter etter type',
    latestActivity: 'Siste aktivitet',
    noActivityYet: 'Ingen aktivitet i denne perioden ennå.',
    periodToday: 'Dag',
    periodWeek: 'Uke',
    periodMonth: 'Måned',
    periodCustom: 'Egendefinert',
    actIncidentReported: 'Hendelse meldt',
    actHazardReported: 'Fare meldt',
    actSuggestionSent: 'Forslag sendt',
    actReportResolved: 'Rapport løst',
    actChecklistDone: 'Sjekkliste fullført',
    actCoursePassed: 'Kurs bestått',
    actCourseAttempt: 'Kursforsøk',
    actProcedureFiled: 'Prosedyre levert',
    actDocumentAdded: 'Dokument lagt til',
    actMemberJoined: 'Nytt medlem',
    actVarslingFiled: 'Varsling mottatt',
    confidential: 'Konfidensielt',
    assignedReports: 'Tildelte rapporter',
    completedTotal: 'Fullført totalt',
    completionRate: 'Fullføringsgrad',
    completedToday: 'Fullført i dag',
    noAdminsFound: 'Ingen administratorer funnet',
    createOrPromoteAdmin: 'Opprett eller promoter en administrator først.',
    seeWhich: 'Se hvilke',
    expectedNote: 'Forventet = dagskvote × kvalifiserende dager. Match et gap mot hvem som hadde skift.',
    latestChecklists: 'Siste sjekklister',
    noRecentChecklists: 'Ingen sjekklister levert i denne perioden.',
    expectedVsReceived: 'Forventet mot mottatt',
    dailyExpected: 'Forventet per dag',
    expectedInRange: 'Forventet i valgt periode',
    receivedLabel: 'Mottatt',
    perDayShort: 'dag',
    more: 'flere',
    showLess: 'Vis færre',
    tapToSeeFiled: 'Trykk for å se når hver ble levert ▾',
    whenFiled: 'Levert',
    allAdmins: 'Alle administratorer',
    completionByAdmin: 'Fullføring per administrator',
    hiddenGroups: 'Skjulte grupper',
    noHiddenGroups: 'Ingen skjulte grupper.',
    restore: 'Gjenopprett',
    permanentDelete: 'Slett permanent',
    requestDelete: 'Be om sletting',
    approveDelete: 'Godkjenn sletting',
    deleteReason: 'Begrunnelse for sletting',
    deleteRequested: 'Sletteforespørsel sendt.',
    deleteApproved: 'Sletteforespørsel godkjent.',
    groupDeleted: 'Gruppe slettet permanent.',
    pendingDeleteRequest: 'Ventende sletteforespørsel',
    pendingDeleteRequests: 'Ventende sletteforespørsler',
    hideGroup: 'Skjul gruppe',
    groupHidden: 'Gruppe skjult.',
    groupRestored: 'Gruppe gjenopprettet.',
    moveToTrash: 'Flytt til papirkurv',
    confirmMoveToTrash: 'Flytt til papirkurv:',
    movedToTrash: 'Flyttet til papirkurv.',
    trashBin: 'Papirkurv',
    trashRetentionHint: 'Slettede elementer blir her i 30 dager for permanent sletting.',
    trashEmpty: 'Papirkurven er tom.',
    trashDayLeft: '1 dag igjen',
    trashDaysLeft: '{count} dager igjen',
    deleteNow: 'Slett nå',
    confirmDeleteTrashNow: 'Slette alle elementer i papirkurven permanent nå? Dette kan ikke angres.',
    trashDeleted: 'Papirkurv slettet.',
    recover: 'Gjenopprett',
    restoredFromTrash: 'Gjenopprettet fra papirkurv.',
    runTrashMigration: 'Kjør migration_trash_soft_delete.sql for å aktivere papirkurven.',
    accountCreatedGrantAccess: 'Konto opprettet. Bekreft e-posten din, og be deretter en admin om tilgang.',
    accountCreatedWaitApproval: 'Konto opprettet. Bekreft e-posten din, og vent på godkjenning fra admin.',
    actionNotePlaceholder: 'Registrer en handling eller et notat...',
    adminNote: 'Adminnotat',
    answerAllQuestions: 'Svar på alle spørsmål ({count} igjen).',
    approved: 'Godkjent',
    assignCourse: 'Tildel kurs',
    assignSelected: 'Tildel valgte',
    assignmentCount: '{count} tildelt',
    assignmentSaved: 'Tildeling lagret.',
    assignments: 'Tildelinger',
    assignmentsRemoved: '{count} tildeling(er) fjernet.',
    attemptCountPassedAverage: '{attempts} forsøk - {passed} bestått - snitt {average}%',
    attempts: 'Forsøk',
    averageScore: 'Gjennomsnittspoeng',
    confirmRead: 'Jeg bekrefter',
    contentGroups: 'Innholdsgrupper',
    deleteChecklistConfirm: 'Slette denne sjekklisten?',
    deleteChecklistLogConfirm: 'Slette denne sjekklisteloggen permanent? Dette kan ikke angres.',
    deleteCourse: 'Slett kurs',
    deleteCourseConfirm: 'Slette dette kurset?',
    deleteReportLogConfirm: 'Slette denne rapportloggen permanent? Dette kan ikke angres.',
    editCourse: 'Rediger kurs',
    emailUsernamePasswordRequired: 'E-post, brukernavn og passord er påkrevd.',
    everyChecklistHitQuota: 'Alle sjekklister traff dagskvoten i denne perioden.',
    executiveAnalytics: 'Ledelsesanalyse',
    icon: 'Ikon',
    latestScore: 'Siste: {score}% {status} - {date}',
    learnerMustTick: 'Deltakeren må krysse av dette for å få poengene.',
    logDeleted: 'Logg slettet.',
    nameRequired: 'Navn er påkrevd.',
    newCourse: 'Nytt kurs',
    noActionsLoggedYet: 'Ingen handlinger loggført ennå.',
    noHiddenReports: 'Ingen skjulte rapporter.',
    noArchivedReportsYet: 'Ingen arkiverte rapporter ennå.',
    noAttempts: 'Ingen forsøk',
    noAttemptsYet: 'Ingen forsøk ennå.',
    noChangeRequestsYet: 'Ingen endringsforespørsler ennå.',
    noChecklistsMatchFilters: 'Ingen sjekklister matcher filtrene.',
    noCompletedCoursesMatchFilters: 'Ingen fullførte kurs matcher filtrene.',
    noCoursesAvailable: 'Ingen kurs tilgjengelig.',
    noCoursesYetAddOne: 'Ingen kurs ennå. Bruk + for å legge til et.',
    noGroupFound: 'Fant ingen {kind}-gruppe.',
    noProfileFound: 'Innloggingen lyktes, men ingen profil ble funnet for denne kontoen.',
    noReportsMatchFilters: 'Ingen rapporter matcher filtrene.',
    noVisibleChanges: 'Ingen synlige endringer',
    noVisibleUsers: 'Ingen synlige brukere funnet.',
    notAssigned: 'Ikke tildelt',
    notPassedNeeded: 'Ikke bestått - {threshold}% kreves',
    nothingChanged: 'Ingenting er endret.',
    openBacklog: 'Åpne saker',
    outcomePlaceholder: 'Resultat og korrigerende tiltak...',
    procedureSubmissions: 'Prosedyreinnsendinger',
    profileChange: 'Profilendring',
    profileChangeHint: 'Endringer sendes til administratorer for godkjenning, som kontoverifisering.',
    profilePendingChange: 'Du har allerede en ventende profilendring. Vent til en admin godkjenner eller avviser den før du sender en ny.',
    rejected: 'Avvist',
    reportsByWorstConsequence: 'Rapporter etter verste konsekvens',
    reportsByRootCause: 'Rapporter etter årsak',
    filteredBy: 'Filtrert etter',
    clearFilter: 'Fjern filter',
    resetFilters: 'Nullstill filtre',
    allRootCauses: 'Alle årsaker',
    filterByRootCause: 'Filtrer etter årsak',
    reviewTryAgain: 'Se gjennom materialet og prøv igjen.',
    selectAll: 'Velg alle',
    selectAssignedUser: 'Velg minst en tildelt bruker.',
    selectUnassignedUser: 'Velg minst en ikke-tildelt bruker.',
    today: 'I dag',
    topType: 'Vanligste type',
    total: 'totalt',
    uniqueUsers: 'Unike brukere',
    yes: 'Ja',
    no: 'Nei',
    username: 'Brukernavn',
    usernameEmailRequired: 'Brukernavn og e-post er påkrevd.',
    usernameTaken: 'Brukernavnet er opptatt.',
    departments: 'Avdelinger',
    departmentName: 'Avdelingsnavn',
    departmentsAdminHint: 'Administrer avdelinger og tildel ansatte nedenfor.',
    visibleToDepartments: 'Synlig for avdelinger',
    departmentsHint: 'Hvis ingen er valgt, er dette elementet synlig for alle.',
    generalDepartmentHint: 'Standardavdeling. Alle brukere tilhører denne avdelingen.',
    departmentMembershipHint: 'Velg hvilke brukere som tilhører denne avdelingen.',
    import: 'Importer',
    importTitle: 'Importer til denne gruppen',
    importHint: 'Velg eksisterende elementer fra andre grupper. De forblir koblet — endrer du ett, oppdateres det overalt.',
    selectItemsToImport: 'Velg elementer å importere',
    noItemsToImport: 'Ingen elementer i andre grupper å importere.',
    itemImported: 'Importert til denne gruppen.',
    itemsImported: 'Importerte {count} element(er) til denne gruppen.',
    linked: 'Koblet',
    removeFromGroup: 'Fjern fra gruppe',
    removedFromGroup: 'Fjernet fra denne gruppen.',
    importSelected: 'Importer valgte',
    fromGroupLabel: 'Fra {group}',
    waitingVerification: 'Venter på godkjenning',
    waitingVerificationBody: 'Kontoen din er opprettet, men en administrator må godkjenne den før du kan bruke appen. Du slipper inn så snart de gjør det.',
    checkAgain: 'Sjekk igjen',
    fullName: 'Fullt navn',
    noAccountsOrChanges: 'Ingen kontoer eller profilendringer venter.',
    noAdminAccess: 'Ingen administratortilgang',
    noAdminAccessBody: 'Du er logget inn som {name}, men dette er administrasjonspanelet. En administrator må gi deg tilgang.',
    assignedToDo: 'Tildelt arbeid',
    assign: 'Tildel',
    nothingAssignedYet: 'Ingenting tildelt ennå.',
    latestSubmitted: 'Siste innsendte',
    noSubmissionsYet: 'Ingen innsendinger ennå.',
    noSubmissions: 'ingen innsendinger',
    lastSubmitted: 'siste',
    addMember: 'Legg til medlem',
    removeMember: 'Fjern',
    confirmDeleteDepartment: 'Slette denne avdelingen? Dette vil fjerne alle medlemmer.',
    selectUserToAssign: 'Velg bruker å tildele...',
    defaultDepartment: 'Standard',
    deleteDepartment: 'Slett avdeling',
    folderVisibility: 'Mappesynlighet',
    saveVisibility: 'Lagre synlighet',
    visibilityTitle: 'Synlighet: {name}',
    visibility: 'Synlighet',
  },
};

export const languages = [
  ['en', 'English'],
  ['no', 'Norsk'],
];

export function initLanguage() {
  const saved = localStorage.getItem(LANG_KEY);
  const lang = TEXT[saved] ? saved : DEFAULT_LANG;
  document.documentElement.lang = lang;
  localStorage.setItem(LANG_KEY, lang);
  return lang;
}

export function currentLanguage() {
  return initLanguage();
}

export function setLanguage(lang) {
  const next = TEXT[lang] ? lang : DEFAULT_LANG;
  localStorage.setItem(LANG_KEY, next);
  document.documentElement.lang = next;
  return next;
}

export function t(key, vars = {}) {
  const lang = currentLanguage();
  const template = TEXT[lang]?.[key] ?? TEXT[DEFAULT_LANG][key] ?? key;
  return Object.entries(vars).reduce((out, [name, value]) => out.replaceAll(`{${name}}`, value ?? ''), template);
}

export function languageSelect(onChange = null) {
  const select = el('select', { class: 'language-select', title: t('language') },
    languages.map(([code, label]) => el('option', { value: code }, label)));
  select.value = currentLanguage();
  select.addEventListener('change', () => {
    setLanguage(select.value);
    if (onChange) onChange(select.value);
  });
  return labeledSelect(t('language'), select);
}

function labeledSelect(label, select) {
  return el('label', { class: 'field language-field' }, [el('span', {}, label), select]);
}

// Format an ISO timestamp as a short local string.
export function fmt(ts) {
  if (!ts) return '';
  const d = new Date(ts);
  return d.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' });
}

// Lightweight toast.
export function toast(msg, kind = 'ok') {
  let host = document.getElementById('toast-host');
  if (!host) {
    host = el('div', { id: 'toast-host', class: 'toast-host' });
    document.body.appendChild(host);
  }
  const t = el('div', { class: `toast toast-${kind}` }, msg);
  host.appendChild(t);
  setTimeout(() => t.classList.add('show'), 10);
  setTimeout(() => { t.classList.remove('show'); setTimeout(() => t.remove(), 300); }, 3200);
}
